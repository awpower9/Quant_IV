...
def update_price(n_clicks, spot, strike, vol_pct, rate_pct, expiry, model,
    ...
    try:
        ...
        if model == "heston":
            ...
            try:
                res_price = grpc_client.price_heston(spot, strike, vol, rate, expiry, is_call, kappa, theta, sigma_v, rho)
                price_text = format_price(res_price.price)
                greeks_div = html.Div([
                    html.Table([
                        html.Thead(html.Tr([html.Th("Greek"), html.Th("Value")])),
                        html.Tbody([
                            html.Tr([html.Td("Delta (Δ)"), html.Td(f"{res_price.greeks.delta:.6f}")]),
                            html.Tr([html.Td("Gamma (Γ)"), html.Td(f"{res_price.greeks.gamma:.6f}")]),
                        ]),
                    ], className="table table-sm table-dark")
                ]) if res_price.HasField("greeks") else html.P("No Greeks available")
            except Exception as e:
                price_text = "Error"
                greeks_div = html.P(str(e))
        ...
        # ─────────────────────────────────────────────────────────────────
            ...
            is_call = option_type.lower() == "call"
            try:
                res_price = grpc_client.price_merton(spot, strike, vol, rate, expiry, is_call, lamb, muj, sigmaj)
                price_text = format_price(res_price.price)
                greeks_div = html.Div([
                    html.Table([
                        html.Thead(html.Tr([html.Th("Greek"), html.Th("Value")])),
