...
engine_core = _quantiv_engine.QuantivPortfolioEngine()
...
def update_comparison_dashboard(n_clicks, spot, strike, vol_pct, rate_pct, expiry, option_type, 
    ...
    try:
        ...
        bsm_res = _quantiv_engine.BlackScholes().price(option, market)
        merton_res = pro_engine.price_merton(option, market, m_lambda, m_muj, m_sigmaj)
        heston_res = pro_engine.price_heston(option, market, h_kappa, h_theta, h_sigmav, h_rho)
        bin_res = _quantiv_engine.Binomial(b_steps).price(option, market)
        ...
        for s in spot_range:
            mkt = _quantiv_engine.MarketData(s, vol, rate)
            p_bsm.append(_quantiv_engine.BlackScholes().price(option, mkt).price)
            p_merton.append(pro_engine.price_merton(option, mkt, m_lambda, m_muj, m_sigmaj).price)
            p_heston.append(pro_engine.price_heston(option, mkt, h_kappa, h_theta, h_sigmav, h_rho).price)
            p_bin.append(_quantiv_engine.Binomial(b_steps).price(option, mkt).price)
        ...
        table = html.Table([
            ...
            html.Tbody([
                html.Tr([
                    ...
                    html.Td(f"{bsm_res.greeks.get('delta', 0.0):.4f}"),
                    html.Td(f"{bsm_res.greeks.get('gamma', 0.0):.4f}"),
                    html.Td("—", className="text-muted")
                ]),
                html.Tr([
                    html.Td("Merton Jump-Diffusion"), 
                    html.Td(format_price(merton_res.price)), 
                    html.Td(f"{merton_res.greeks.get('delta', 0.0):.4f}"),
                    html.Td(f"{merton_res.greeks.get('gamma', 0.0):.4f}"),
                    html.Td(get_diff(merton_res.price, bsm_res.price))
                ]),
                html.Tr([
                    html.Td("Heston Stochastic Vol"), 
                    html.Td(format_price(heston_res.price)), 
                    html.Td(f"{heston_res.greeks.get('delta', 0.0):.4f}"),
                    html.Td(f"{heston_res.greeks.get('gamma', 0.0):.4f}"),
                    html.Td(get_diff(heston_res.price, bsm_res.price))
                ]),
                html.Tr([
                    html.Td(f"Binomial Tree ({b_steps} steps)"), 
                    html.Td(format_price(bin_res.price)), 
                    html.Td(f"{bin_res.greeks.get('delta', 0.0):.4f}"),
                    html.Td(f"{bin_res.greeks.get('gamma', 0.0):.4f}"),
